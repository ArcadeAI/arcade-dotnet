...
[JsonConverter(
    typeof(JsonModelConverter<ScheduledListPageResponse, ScheduledListPageResponseFromRaw>)
)]
public sealed record class ScheduledListPageResponse : JsonModel
{
    public IReadOnlyList<ToolExecution>? Items
    {
        get { return JsonModel.GetNullableClass<List<ToolExecution>>(this.RawData, "items"); }
        init
        {
            if (value == null)
            {
...
class ScheduledListPageResponseFromRaw : IFromRawJson<ScheduledListPageResponse>
{
    /// <inheritdoc/>
