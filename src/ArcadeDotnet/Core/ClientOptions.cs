...
public record struct ClientOptions()
{
    ...
    Lazy<string> _baseUrl = new(() =>
        Environment.GetEnvironmentVariable("ARCADE_BASE_URL") ?? EnvironmentUrl.Production
    );
    ...
    /// <see cref="IArcadeClient.WithRawResponse"/>, don't perform validation until deserialization
    /// is triggered.</para>
    /// </summary>
