...
public class ToolServiceTest : TestBase
{
    ...
    {
        var page = await this.client.Tools.List(new(), TestContext.Current.CancellationToken);
        page.Validate();
    }
    ...
    {
        var executeToolResponse = await this.client.Tools.Execute(
            new() { ToolName = "tool_name" },
            TestContext.Current.CancellationToken
        );
    ...
    {
        var toolDefinition = await this.client.Tools.Get(
            "name",
            new(),
            TestContext.Current.CancellationToken
        );
