...
using ArcadeEngine.Models.Admin.AuthProviders.AuthProviderCreateParamsProperties.Oauth2Properties;

namespace ArcadeEngine.Models.Admin.AuthProviders.AuthProviderCreateParamsProperties;
        ...
        get
        {
            if (!this.Properties.TryGetValue("client_id", out JsonElement element))
                throw new ArcadeInvalidDataException(
                    ...
                    new ArgumentOutOfRangeException("client_id", "Missing required argument")
                );

            return JsonSerializer.Deserialize<string>(element, ModelBase.SerializerOptions)
                ?? throw new ArcadeInvalidDataException(
                    "'client_id' cannot be null",
                    new ArgumentNullException("client_id")
                );
        }
        set
        {
