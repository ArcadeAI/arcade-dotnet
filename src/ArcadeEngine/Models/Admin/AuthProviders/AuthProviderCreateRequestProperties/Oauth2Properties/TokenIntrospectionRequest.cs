...
using ArcadeEngine.Models.Admin.AuthProviders.AuthProviderCreateRequestProperties.Oauth2Properties.TokenIntrospectionRequestProperties;

namespace ArcadeEngine.Models.Admin.AuthProviders.AuthProviderCreateRequestProperties.Oauth2Properties;
        ...
        get
        {
            if (!this.Properties.TryGetValue("triggers", out JsonElement element))
                throw new ArcadeInvalidDataException(
                    ...
                    new ArgumentOutOfRangeException("triggers", "Missing required argument")
                );

            return JsonSerializer.Deserialize<Triggers>(element, ModelBase.SerializerOptions)
                ?? throw new ArcadeInvalidDataException(
                    "'triggers' cannot be null",
                    new ArgumentNullException("triggers")
                );
        }
        set
        {
